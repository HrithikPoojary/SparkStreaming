class invoiceStreamBatch():
    ...
    def appendFlattenDf(self, flattendf , trigger = 'batch'):
        sQuery = (
                    flattendf.writeStream.format("delta")
                                        .option("checkpointLocation" , self.checkpoint_path)
                                        .outputMode("append")
                                        .option("maxFilesPerTrigger" , 1)  # processingTime -> for each run we will process only 1 file ( if there are 10 files it will process only 1 at first run next 2 file so on...) default - 1000 
                                                                           # for availableNow -> all files(10 files) will processed before streaming query stops.It will not stop unless until all data processed it might run 1 or more micro batch                                      
                 )
        if trigger == 'batch':
            return ( 
    ...
    def main(self , trigger = 'batch'):
        ...
        sQuery = self.appendFlattenDf(result, trigger)
        print("Done \n")
        return sQuery
