class Brownze():
    ...
    def upsert(self , batchdf , batchid):
        batchdf.createOrReplaceTempView('user_invoice_df')

        merge_statement = """
                            merge into invoice_bz bz
                            using user_invoice_df df
                            on bz.value == df.value
                            and bz.timestamp == df.timestamp
                            when matched then
                                update set *
                            when not matched then
                                insert *
        """
        #set * ->update all records with new value
        #insert * ->insert the new record
    ...
    def main(self , startingtime = 1):
        ...
        sQuery = (
                    invoice_df.writeStream.format("delta")
                                          ...
                                          .foreachBatch(self.upsert)         #foreachBatch ->  for each microbatch i pass the dataframe into the user defined function it should always takes two parameter
                                          .outputMode("append")              #foreachBatch -> 2 parameters (batchdf(invoice_df) , batchid (evevy microbatch some unique id like 0,1,2,3)
